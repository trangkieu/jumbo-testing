...
describe('Movie details', () => {
    ...
    it('year release details', () => {
        divs.map(d => {
                ...
                if (d.props.id === 'movie-title') {
                    expect(d.props.children.toString().replace(/,/g, '')).toEqual("Bohemian Rhapsody");
                    yearReleaseCount++;
                }
                if (d.props.id === 'rating') {
                    expect(d.props.children.toString().replace(/,/g, '')).toEqual("2018 · 80% User Score");
                    votePercentageCount++;
                }
